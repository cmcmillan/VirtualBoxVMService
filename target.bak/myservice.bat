...
set DISPLAYNAME="VM: %VM_NAME%"

if "%1" == "" goto displayUsage
if "%2" == "" goto setServiceName
set VM_NAME=%2
set SERVICE_NAME=VM_%2
set DISPLAYNAME="VM: %2"
...
rem if "%STOP_METHOD%" == "" set START_METHOD=-method %START_METHOD%
...
:foundJvm
echo Using JVM:              %JVM%
echo Using install command:  %EXECUTABLE% -install %SERVICE_NAME% %JVM% -Djava.class.path=%CLASS_PATH% %START_ARGS% %STOP_ARGS% -description %DISPLAYNAME% 
rem -current "%CURRENT_DIR%"

echo .
echo Install the service
rem Install the service
"%EXECUTABLE% -install %SERVICE_NAME% %JVM% -Djava.class.path=%CLASS_PATH% %START_ARGS% %STOP_ARGS% -description %DISPLAYNAME%" rem -current "%CURRENT_DIR%"
